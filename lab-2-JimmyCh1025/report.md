...
- Vgg8 :
    ...
    Output Shape: torch.Size([1, 192, 16, 16])

    Layer: MaxPool2d
    Input Shape: torch.Size([1, 192, 16, 16])
    Output Shape: torch.Size([1, 192, 8, 8])

    Layer: Conv2d
    Input Shape: torch.Size([1, 192, 8, 8])
    Output Shape: torch.Size([1, 384, 8, 8])

    Layer: Conv2d
    Input Shape: torch.Size([1, 384, 8, 8])
...
|                          |   vgg8   | vgg8-power2 |
| ------------------------ | -------- | -------- |
| total inference times    | 5.112 ms | 1.960 ms |


分析： 在 CPU 模擬環境下，Power2 版本約 2.6 倍的加速，這個倍數回隨著硬體改變有所變動。但這也證明了將權重限制為 2 的冪後，軟體層級透過簡化算術邏輯與減少記憶體頻寬壓力，推論效率有顯著提升。

B. 運算算子與硬體映射 (Operations and Mapping)


算子差異： 
|               |   vgg8   | vgg8-power2 |
| ------------- | -------- | -------- |
| Operations    | aten::mkldnn_convolution | quantized::conv2d_relu |

=> 運算從傳統的 乘加運算 (MAC) 轉變為 Shift 與加法。

Mapping 邏輯： 雖然運算改變，但兩者的 Input Shapes 完全相同，也就是兩者會產生相同的資料流映射參數 (m,n,e,p,q,r,t)。意味著資料在 GLB 與 PE 之間的搬運路徑是一致的，效能完全來自於 PE 的簡化。
...
Calculation:
* Total MACs: $N \times M \times E \times F \times C \times R \times S = 1 \times 64 \times 32 \times 32 \times 3 \times 3 \times 3 = 1,769,472$
* Total Memory Traffic:
  $Weights = R \times S \times C \times M = 3 \times 3 \times 3 \times 64 = 1,728$
  $Ifmap = H \times W \times C \times N = 32 \times 32 \times 3 \times 1 = 3,072$
  $Ofmap = E \times F \times M \times N = 32 \times 32 \times 64 \times 1 = 65,536$
  $Total\ Traffic = 1,728 + 3,072 + 65,536 = 70,336 \text{ bytes}$
* Ideal Operational Intensity (OI):
  $OI = 1,769,472 \div 70,336 \approx 25.16 \text{ MACs/byte}$

因為理想 OI (25.16) 大於硬體的 Machine Balance (48/4 = 12)，因此在此硬體配置下，卷積層處於 Compute-bound。
