`include "src/PE_array/GIN/GIN_Bus.sv"
`include "src/PE_array/GIN/GIN_MulticastController.sv"

module GIN (
    input clk,
    input rst,

    // Slave SRAM <-> GIN
    input GIN_valid,
    output logic GIN_ready,
    input [`DATA_BITS - 1:0] GIN_data,

    /* Controller <-> GIN */
    input [`XID_BITS - 1:0] tag_X,
    input [`YID_BITS - 1:0] tag_Y,

    /* config */
    input set_XID,
    input [`XID_BITS - 1:0] XID_scan_in,
    input set_YID,
    input [`YID_BITS - 1:0] YID_scan_in,

    // Master GIN <-> PE
    input [`NUMS_PE_ROW * `NUMS_PE_COL - 1:0] PE_ready,
    output logic [`NUMS_PE_ROW * `NUMS_PE_COL - 1:0] PE_valid,
    output logic [`DATA_BITS - 1:0] PE_data
);
    /*
                    ____  ____  ____  ____       ____  ____  ____ 
                   | MC || MC || MC || MC |     | MC || MC || MC |
        ||          ----  ----  ----  ----       ----  ----  ----
        ||    ____   ||    ||    ||    ||   ...   ||    ||    || 
        || = | MC | ================================================ X-Bus1
        ||    ----
        ||     .    ____  ____  ____  ____       ____  ____  ____ 
        ||     .   | MC || MC || MC || MC |     | MC || MC || MC |
        ||     .    ----  ----  ----  ----       ----  ----  ----
        ||    ____   ||    ||    ||    ||   ...   ||    ||    || 
        || = | MC | ================================================ X-Bus12
        ||    ----  
       Y-Bus
    */

    /*
         master                                               slave
         -----  GLB_valid       Y_Bus_valid        X_Bus_valid
        | GLB |     ->  | Y_Bus |    ->   | X_Bus |    ->    | PE |
         -----      <-               <-                <- 
                Y_Bus_ready     X_Bus_ready        PE_ready
    */

    logic [`NUMS_PE_ROW-1 : 0] X_Bus_valid;
    logic [`NUMS_PE_ROW-1 : 0] X_Bus_ready;

    logic [`XID_BITS-1:0] MC_id [`NUMS_PE_ROW-1:0];
    logic [`XID_BITS-1:0] MC_id_out [`NUMS_PE_ROW-1:0];


    GIN_Bus #(
        .NUMS_SLAVE  (`NUMS_PE_ROW),
        .ID_SIZE     (`YID_BITS)
    ) GIN_Y_Bus (
        .clk         (clk),
        .rst         (rst),
        .tag         (tag_Y),
        .master_valid(GIN_valid),   // input
        .master_data (GIN_data),    // input
        .master_ready(GIN_ready),   // output
        .slave_ready (X_Bus_ready), // input
        .slave_valid (X_Bus_valid), // output
        .slave_data  (PE_data),     // output filter / ifmap / psum
        .set_id      (set_YID),
        .ID_scan_in  (YID_scan_in),
        .ID_scan_out ()             // Global input network => no output
    );


    generate 
        for (genvar i = 0 ; i < `NUMS_PE_ROW ; ++i) begin : gin_bus_gen
            GIN_Bus #(
                .NUMS_SLAVE  (`NUMS_PE_COL),
                .ID_SIZE     (`XID_BITS)
            ) GIN_X_Bus (
                .clk         (clk),
                .rst         (rst),
                .tag         (tag_X),
                .master_valid(X_Bus_valid[i]),                             // input
                .master_data (),                                           // input
                .master_ready(X_Bus_ready[i]),                             // output
                .slave_ready (PE_ready[i * `NUMS_PE_COL +: `NUMS_PE_COL]), // input
                .slave_valid (PE_valid[i * `NUMS_PE_COL +: `NUMS_PE_COL]), // output
                .slave_data  (),                                           // output
                .set_id      (set_XID),
                .ID_scan_in  (MC_id[i]),
                .ID_scan_out (MC_id_out[i])
            );
        end
    endgenerate

    always_comb begin
        MC_id[0] = XID_scan_in;
        for (int i = 1 ; i < `NUMS_PE_ROW ; ++i) begin
            MC_id[i] = MC_id_out[i-1];
        end
    end

endmodule
